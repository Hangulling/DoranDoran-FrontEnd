export const validateName = (name: string): string | null => {
  const v = name.trim()
  if (v.length > 15) return 'Enter 1-15 characters'
  return null
}

export const EMAIL_REGEX_ASCII =
...
export const isAtLeast14YearsOld = (birthDate: string) => {
  const y = Number(birthDate.slice(0, 4))
  const m = Number(birthDate.slice(4, 6))
  const d = Number(birthDate.slice(6, 8))
  ...
  const valid = birth <= limit

  if (!valid) {
    return 'You must be 14 or older to sign up'
  }

  return null
}

export const validateBirthDate = (digits: string): string | null => {
  if (digits.length < 8) return 'Please enter the date of birth in 8 digits'
  if (digits.length > 8) return 'Please enter the date of birth in 8 digits'

  const cal = isValidCalendarDate(digits)
  if (cal) return cal

  const past = isPastDate(digits)
  if (past) return past

  const age = isAtLeast14YearsOld(digits)
  if (age) return age
