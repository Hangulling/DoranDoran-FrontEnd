...
const ChatRoomItem = ({ room, onClick }: ChatRoomItemProps) => {
  const MainData = MAIN_DATA.find(data => data.roomName === room.roomName)
  if (!MainData) return null

  return (
    <button
      onClick={() => onClick(MainData.roomRouteId, MainData.roomName)}
      className="flex items-center gap-3 w-full h-[57px] bg-gray-0 py-[6px]"
    >
      <div className="w-10 h-10 rounded-[12px] flex items-center justify-center overflow-hidden">
        <img
          src={MainData.avatar}
          alt={MainData.roomName}
          className="w-full h-full object-cover"
        />
      </div>
      <div className="flex flex-col items-start">
        <span className="text-title text-[16px]">
          {capitalizeFirstLetter(MainData.roomName)}
        </span>
